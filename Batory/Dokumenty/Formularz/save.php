...
if ($idd==0)
{
	...
	mysqli_query($link, $q="update dokumentz set ID_D='$idd' where ID_D=-1 and KTO='$ido'");if (mysqli_error($link)) {die(mysqli_error($link).'<br>'.$q);}
...
if ($idd)
{
	...
	elseif(mysqli_fetch_row(mysqli_query($link, $q="select count(*) from dokumentm where ID_D=$idd"))[0]<>0)
	{
		$od_netto=$_SESSION['od_netto'];
		require("{$_SERVER['DOCUMENT_ROOT']}/Batory/Towary/przelicz.php");
	}
...
/*
		...
		if(!$kp)
		{
			$kp=mysqli_fetch_row(mysqli_query($link, $q="select NUMER from dokumenty where ID<$idd and TYP='KP' order by ID desc limit 1"))[0];
			if(substr($kp,-4,4)<>date('Y'))
			{
				$kp='1/'.date('Y');
			}
			else
			...
			mysqli_query($link, $q="update dokumenty set DODOK='$kp', DTERMIN=DDOKUMENTU, KTO='$ido', CZAS=Now() where ID=$idd");if (mysqli_error($link)) {die(mysqli_error($link).'<br>'.$q);}
		}
*/
	}
	else
	{
		mysqli_query($link, $q="update dokumenty set DODOK='', DTERMIN=if(DTERMIN*1=0,Date_Add(DDOKUMENTU,interval 7 day),DTERMIN), KTO='$ido', CZAS=Now() where ID=$idd");if (mysqli_error($link)) {die(mysqli_error($link).'<br>'.$q);}
	}
