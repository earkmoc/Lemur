...
//	$('select[name=TYP]').focus();
	...
	$('#myModalN').on('shown.bs.modal', function () {
		$('#iframeKontrahenciN').focus();
	});
	$('#myModalO').on('shown.bs.modal', function () {
		$('#iframeKontrahenciO').focus();
	});
	...
	$('#myModalRozliczenia').on('shown.bs.modal', function () {
		$('#iframeRozliczenia').focus();
	});
	...
	$("input[name="+$name+"]").load("valid.php",$params,function(){
		...
		else
		{
			...
			if($wynik!="")
			{
				...
				{
					$('#myModalN').modal('show');
					$('#iframeKontrahenciN').attr( 'src', function ( i, val ) { return val; });
				}
			}
		}
	});
...
function GetKontrahentByNIPO($name,$validType)
{
	$value=$("input[name="+$name+"]").val();
	$params="val="+$value+"&validType="+$validType;
	$("input[name="+$name+"]").load("valid.php",$params,function(){
		...
		else
		{
			...
			{
				var arr=$wynik.split(",");
				$("input[name=DELIVERY]").val(arr[0]);
				$("input[name=PAYMENT]").val(arr[1]);
				if(arr[2]>1)
				{
					$('#myModalO').modal('show');
					$('#iframeKontrahenciO').attr( 'src', function ( i, val ) { return val; });
				}
			}
		}
	});
...
function GetKontrahentByNAZWAO($name,$validType)
{
	$value=$("input[name="+$name+"]").val();
	$params="val="+$value+"&validType="+$validType;
	$("input[name="+$name+"]").load("valid.php",$params,function(){
...
function valid($name,$validType) 
{
	...
	{
		$title="Zachowaj regu³ê:<br>Data wystawienia >= Data Sprzeda¿y";
		RegulaMR('DOPERACJI','DDOKUMENTU',$title);
		RegulaWR('DDOKUMENTU','DOPERACJI',$title);
	}
	if($validType=='NIP') 		{GetKontrahentByNIP($name,$validType);}
	if($validType=='NIPO') 		{GetKontrahentByNIPO($name,'NIP');}
	if($validType=='NAZWA') 	{GetKontrahentByNAZWA($name,$validType);}
	if($validType=='NAZWAO') 	{GetKontrahentByNAZWAO($name,'NAZWA');}
