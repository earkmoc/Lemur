...
*/
	...
	{
		$tabele[]='dokumentz';
		mysqli_query($link,$q="drop table dokumentz");
		$towaryStruktura=mysqli_fetch_row(mysqli_query($link,$q="select STRUKTURA from Lemur2.tabele where NAZWA='dokumentz'"))[0];
		mysqli_query($link,$towaryStruktura);
	}
...
//KNORDPOL
	...
	$pliki=array(
		...
		,'rej_sk'=>'rejestr sprzeda¿y VAT - kaucje'
...
//										break;
							...
							case $skrot=='towaryk':
								switch ($key)
								{
									case 'LPP':
										$key='ID_D';
										$value=((1*$value<0)?'0':1*$value+$lpDelta);
										break;
									case 'ILOSC':
										$ilosc=$value;
										break;
									...
									case 'CENA_PUP':
										$cena=$value;
										$key="NAZWA=concat('$nazwa1',if('$nazwa2'<>'','|$nazwa2',''),if('$nazwa3'<>'','|$nazwa3',''))".", CENABEZR='$cena', CENA";
										break;
									case 'BRUTTO':
										$value=($value?$value:'0');
										$netto=$value;
										$podatek=round($netto*0.01*$stawka,2);
										$brutto=$netto+$podatek;
										$value=$brutto;
										$value=($value?$value:'0');
										$key="VAT='$podatek', NETTO='$netto', BRUTTO";
										break;
									case 'INDEKS':
									case 'SWWKU':
										$key='';
								...
								$q.=(!$q?"insert into dokumentm set TYP='T'":'');
								$q.=($key?($value==null?", $key":", $key='$value'"):'');
//								echo $q;
								break;
							case $skrot=='splaty':
							case $skrot=='splatyk':
								...
								$q.=(!$q?"insert into dokumentz set KTO='$ido'":'');
								$q.=($key?($value===null?", $key":", $key='$value'"):'');
								break;
							case $skrot=='towary':
								switch ($key)
							...
							case $skrot=='rej_sk':
								switch ($key)
								{
									case 'LP':
										...
										break;
									case 'NUMER':
										$key="TYP='".($typ=='FV'?(strpos($value,'J')?'FJ':'FV'):$typ)."', NUMER";
										break;
									case 'D1':
										$key='DWPLYWU';
										break;
								...
								if($key)
								{
									$q.=(!$q?"insert into dokumenty set KTO='$ido', CZAS=Now(), GDZIE='ksiêgi', $key='$value'":", $key='$value'");
								}
								break;
							case $skrot=='rej_ka':
								switch ($key)
								{
									case 'NUMER':
										$key="TYP='".(strtoupper(substr($value,0,2)))."', NUMER";
										break;
									case 'D1':
										$key='DWPLYWU';
										break;
									...
									case 'D4':
										...
										break;
									case 'K10':
										$key=($value*1<>0?"WPLACONO='$value', WARTOSC":'');
										break;
									case 'K11':
										$key=($value*1<>0?"WPLACONO='$value', WARTOSC":'');
										break;
								...
								if($key)
								{
									$q.=(!$q?"insert into dokumenty set KTO='$ido', CZAS=Now(), GDZIE='ksiêgi', SPOSZAPL='gotówka', $key='$value'":", $key='$value'");
								}
								break;
							case $skrot=='rej_zk':
								switch ($key)
...
*/
	...
	*/
		...
		$w=mysqli_query($link,$q="
			update dokumentr
			   set STAWKA='23%'
			 where STAWKA not IN ('23%','8%','7%','5%','3%','2%','zw.','0%')
		");
		if (mysqli_error($link)) {die(mysqli_error($link).'<br>'.$q);}
	}
