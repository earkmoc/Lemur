...
$poczLP=mysqli_fetch_row(mysqli_query($link,$q="
	select LP
	  from kpr
	 where left(DATA,7)<'$okres'
  order by DATA desc, LP*1 desc
	 limit 1
"))[0]+1;
if (mysqli_error($link)) {die(mysqli_error($link).'<br>'.$q);}
...
echo '<br>';
echo 'Od jakiego LP zacz±æ? (puste oznacza kontynuacjê LP z poprzedniego okresu): <input class="form-control" name="poczLP" value="'.$poczLP.'" >';
echo '</span>';
echo '</div>';
