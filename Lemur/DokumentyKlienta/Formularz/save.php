...
//pobranie NIP i nazwa kontrahenta z pól menu głównego
...
if ($idd)
{
	...
	$w=mysqli_query($link, $q="select NUMER, TYP, DDOKUMENTU from dokumenty where ID=$idd");
	if	(	($r=mysqli_fetch_array($w))
		&&	($r['NUMER']=='automat')
		)
	{
		require("{$_SERVER['DOCUMENT_ROOT']}/Lemur2/NextNumer.php");
		$numer=NextNumer($link, $r['TYP'],$r['DDOKUMENTU'],'', $idd);
		mysqli_query($link, $q="update dokumenty set NUMER='$numer' where ID=$idd");
	}
	mysqli_query($link, $q="update dokumenty set NIP='{$_POST['NIP']}', NAZWA='{$_POST['NAZWA']}' where ID=$idd");
