...
$w=StripSlashes($w[0]);	//tekst do zmiany (ten zapis musi być na końcu)
...
fputs($file,'<script type="text/javascript" src="'."http://{$_SERVER['HTTP_HOST']}/Lemur2/".'js/jquery-1.10.2.min.js"></script>'."\n");
...
while ($wiersz=mysqli_fetch_array($wynik)) {
	...
	$naglow=($f=='n');
	$stopka=($f=='s');
	if ($f=='+'||$naglow||$stopka) {	//subtabela, np.: specyfikacja, nagłówek, stopka
		$lps=0;				//l.p. w subtabeli
		$lps2=0;			//l.p. w subsubtabeli
		$bloklp=0;			//l.p. w ramach bloku
		if (trim($z)=='') {	//nie było żadnych zapytań, więc jeden wiersz (pewnie nagłówek lub stopka)
			$lpsmax=1;
		} else {
			require('WydrukWzorSeria.php');
			if (count($qs)>1) {	//więcej pól wyniku, to znaczy, że są tam pola dla specyfikacji w formacie [1], [2], itd.
				$lpsmax=-1;	//i nie wiadomo ile będzie wierszy
				$blok0=$qs[0];	//wartość poprzedniego wiersza w ramach bloku
				$blok1=$qs[0];	//wartość bieżącego wiersza w ramach bloku
			} else {		//tylko jedno pole wyniku, to znaczy, że 
				$lpsmax=$qs[0];	//wynik ostatniego zapytania to ilość wierszy do obróbki
			}
		}	//if (trim($z)!='')
	   ...
	   while ($lpsmax==-1 ? (1==1) : ($lps<$lpsmax)) {
		...
		if (count(explode('[lp]',$ws))>0) {	//jeśli '[lp]' jest już w tekście głównym specyfikacji
			$ws=str_replace('[lp]',$lps+1,$ws);
		}

		if ($lpsmax<0) {		//więcej pól wyniku, to znaczy, że są tam pola dla specyfikacji w formacie [1], [2], itd.
			for ($j=0;$j<count($qs);$j++) {		//korzysta z wyników zapytania głównego specyfikacji
				$ws=str_replace('['.$j.']',$qs[$j],$ws);
			}
			$blok1=$qs[0];
			if ($blok0!=$blok1&&!$blokbreak) {	//zmienił się numer bloku i nie łamał strony ledwo co
			...
			$blok0=$qs[0];
			if (count(explode('[bloklp]',$ws))>0) {//jeśli '[bloklp]' jest już w tekście głównym specyfikacji
		...
		while ($wierszs=mysqli_fetch_array($wyniks)) {	//lecimy po polach do zmiany
			...
			if ($f=='+') {		//subraport2
				require('WydrukWzorSeria.php');
				$fvs2=$n;	//słowo: FakturaPS1Spec
				$lps2=0;
				$lps2max=$qs[0];//ilość wierszy do obróbki
				while ($lps2<$lps2max) {
...
//echo '<br>';
	   ...
	   if ($naglow) {$blokn=$ws;}		//mamy nagłówek
	   if ($stopka) {$bloks=$ws;}		//mamy stopkę
	}	//if ($f=='+')
	else {
